...
@section('content')
    ...
    $reportOP = report_items_openvas::where('IP', $NAME)->get();
    $reportNES = report_items_nessus::where('Host', $NAME)->get();
    ...
    function getCVSSEVNVI($row){
        ...
        foreach ($rowCalc as $item){
            $ENVI = app('\App\Http\Controllers\cvss_openvasController')->calculateCVSS_ENVI($item->id);
            return $ENVI;
        }
    ...
    function getCVSSTEMP($row){
        ...
        $rowCalc = \App\CVSS_OpenVas::where('idRow', $row->id)->get();
        foreach ($rowCalc as $item){
            $TEMP = app('\App\Http\Controllers\cvss_openvasController')->calculateCVSS_TEMP($item->id);
            return $TEMP;
        }
    ...
    <h2 align="center">Výpis záznamů pro {{$NAME}}</h2>
