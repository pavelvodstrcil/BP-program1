...
@section('content')
    ...
    foreach ($ipadresses as $ip){
        ...
        if ($ip_parts[0] == $NET and $ip_parts[1] == $SUBNET ){
            if (!in_array($ip_parts[2], $IP_part1)){
                array_push($IP_part1, $ip_parts[2]);
    ...
    <h2 align="center">Vyberte prosím další kus sítě: z {{$NET}}.{{$SUBNET}}.0.0 </h2>
    <h3 align="center">Z reportů byly detekovány tyto sítě:</h3>
    <table class="table">
        <thead>
        <tr>
            <th scope="col">Síť</th>
            <th scope="col">Nejhorší OpenVas</th>
            <th scope="col">Nejhorší Nessus</th>
        ...
        <tr>
            ...
            @foreach($IP_part1 as $row)
                ...
                <td><a href="{{$SUBNET}}/{{$row}}" >{{$NET}}.{{$SUBNET}}.{{$row}}.0</a></td>
